...
- **FR-013**: At token exchange time, for each service **in the agent's `service_requirements`**, the system MUST compute the union of scopes from all granted permission sets whose `ServiceScope` entries cover that service **AND** whose `included_service_ids` contain that service. The resulting PS-derived scopes MUST then be **intersected** with the agent's `ServiceRequirement.scopes` for that service — SR.scopes acts as a per-agent ceiling, silently capping PS scopes that exceed the agent's allowed boundary. Services covered by granted PermissionSets but NOT in the grant's `included_service_ids` are ignored. Services covered by PS but NOT in the agent's `service_requirements` are also ignored. The resulting per-service effective scopes are used to validate the existing `UserSession` token and to build the access token.
...
- **FR-017**: During token exchange, the system MUST resolve `granted_permission_sets` → effective scopes using an in-process cache (≈60s TTL), applying SR scope ceiling intersection per FR-013. It MUST validate that the existing `UserSession` token already covers the resulting effective scopes — it MUST NOT expand or re-request scopes.
...
- **ServiceRequirement** (unchanged structure, clarified role): Retains its existing structure — `{service_id, scopes[], requirement_type: "mandatory"|"optional"}`. No permission set references. Serves two distinct roles: (1) **session gating** — defines which services need active `UserSession`s and whether they're mandatory/optional for the agent to function; (2) **scope ceiling** — `scopes[]` defines the maximum allowed scopes per service for this agent. At token exchange, PS-derived scopes are intersected with SR scopes (FR-013). PS scopes may be broader (PS are global); SR caps the effective boundary per agent.
