...
`authorization_params` is optional. Omission or an empty object means the provider receives no additional static authorization-request parameters.
...
Values originate only from stored administrator configuration. The end-user authorization-start endpoint does not accept or forward arbitrary provider parameters. Broker-generated OAuth2 fields remain authoritative.
