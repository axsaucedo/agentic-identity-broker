...
| `broker_client_id` | `BrokerClientID` (string) | UNIQUE, NOT NULL | Opaque client identifier for OAuth2 flows |
...
- `broker_client_id` must be non-empty, globally unique
...
Opaque string identifier uniquely identifying a broker-issued OAuth2 client. Format: `broker_` prefix + 22 character base64url random string (e.g., `broker_7k4Hx9pQ2mLv3nWs5tYz1a`). Immutable after generation.

**Type**: `type BrokerClientID string` in `internal/domain/id/string_ids.go`
...
|---|---|---|
| `BrokerClientID` | OAuth2 client identifier for broker-issued credentials | `internal/domain/id` |
