...
- [ ] T001 Run `just build` and `just test` to confirm all existing tests pass before any changes; record baseline
...
- [ ] T002 Confirm no new glossary terms are required — `MatchesRedirectURI` is an internal function, not a domain concept; `RedirectURIValidator` is already documented in the spec. Mark 2a complete.
...
- [ ] T003 Confirm N/A — this feature adds no new configuration parameters, modifies no existing ones, and requires no Helm chart update.
...
- [ ] T004 Confirm N/A — this feature makes no changes to any API endpoint, request/response shape, or OpenAPI spec. Validation is purely internal.
...
- [ ] T005 Confirm N/A — no schema changes, no new entities, no migration files required.
...
- [ ] T006 Review `tests/e2e/frontend/consent_flow_test.go` and `tests/e2e/pages/` to identify the existing loopback warning test and which page object method drives the localhost redirect warning (CS-003 from 028)
- [ ] T007 [P] Confirm the screenshot filename convention: `tests/e2e/screenshots/consent_loopback_warning_portless.png` aligns with existing naming in `tests/e2e/screenshots/`
...
- [ ] T008 Create `tests/e2e/cimd_redirect_uri_test.go` with one `It()` block per acceptance scenario — 10 scenarios total:
...
- [ ] T009 [P] Create `tests/e2e/portless_opaque_test.go` with one `It()` block for SC-006: opaque (UUID) `client_id` Agent with `http://localhost:3000/callback` in `redirect_uris`, authorization request with `redirect_uri=http://localhost:9999/callback` → `Expect(resp.StatusCode).To(Equal(http.StatusFound))`; include `// SC-006 from specs/028b-portless-registration/spec.md`
- [ ] T010 Run `go build ./tests/e2e/...` — all new test files MUST compile with no errors (create minimal CIMD document fixtures as needed to make them compile)
- [ ] T011 Run `ginkgo -v ./tests/e2e/` — all new `It()` blocks MUST fail semantically with realistic assertions (e.g., "Expected 400 to equal 302"), NOT compilation errors or placeholder failures; record failure output as proof of red phase
- [ ] T012 Amend `tests/e2e/frontend/consent_flow_test.go` to add/update one `It()` block for SC-005: loopback warning displayed when registered URI is portless (`http://localhost/callback`) and runtime URI includes a port; assert warning element visibility using existing page object; add `// SC-005 from specs/028b-portless-registration/spec.md` comment
- [ ] T013 [P] Verify `tests/e2e/frontend/consent_flow_test.go` screenshot save call targets `tests/e2e/screenshots/consent_loopback_warning_portless.png`
- [ ] T014 Verify Playwright test for SC-005 fails before implementation (red phase)
...
- [ ] T015 Add `TestMatchesRedirectURI` table-driven test function to `internal/domain/storage/agent_test.go` with the 12 cases from `specs/028b-portless-registration/quickstart.md` Step 1 (portless loopback → match, explicit-port different port → match, explicit-port portless request → match, path mismatch → no match, scheme mismatch → no match, localhost vs 127.0.0.1 → no match, non-loopback same → match, non-loopback port differs → no match, non-loopback portless+port → no match, plus edge cases)
- [ ] T016 Run `go test ./internal/domain/storage/...` — `TestMatchesRedirectURI` MUST fail to compile (function does not exist yet); confirm red phase
- [ ] T017 Add `MatchesRedirectURI(registered, incoming string) bool` to `internal/domain/storage/agent.go` immediately below `IsValidRedirectURI`, implementing the loopback port-ignore logic per `specs/028b-portless-registration/quickstart.md` Step 2
- [ ] T018 Run `go test ./internal/domain/storage/...` — `TestMatchesRedirectURI` MUST now pass; all pre-existing tests MUST remain green
- [ ] T019 Replace the `==` string equality in the redirect URI matching loop in `internal/domain/oauth2/service.go` (~lines 177-183) with `storage.MatchesRedirectURI(allowed, req.RedirectURI)`; `internal/domain/storage` is already imported — no import change needed
- [ ] T020 Replace the `v == item` equality in the `contains()` helper in `internal/domain/oauth2server/provider.go` with `storage.MatchesRedirectURI(v, item)`; add `internal/domain/storage` import if not already present
- [ ] T021 Run `go test ./internal/domain/oauth2/... ./internal/domain/oauth2server/...` — all pre-existing tests MUST pass green; confirm no regressions introduced
...
- [ ] T022 [US1] Run `ginkgo -v --label-filter="US1" ./tests/e2e/cimd_redirect_uri_test.go` (or equivalent focus) — US1 Scenarios 1, 2, 3 MUST now pass (portless registered URI accepted with any port); Scenario 4 (path mismatch) MUST still reject
- [ ] T023 [US1] Confirm SC-001 satisfied: three distinct port scenarios verified (port 52341, port 8080, portless) — all green
...
- [ ] T024 [US2] Run US2 E2E scenarios 1–3 — explicit-port registered URI matched against different ephemeral ports and portless requests; all three MUST pass (same `MatchesRedirectURI` implementation, different registered URI shape)
- [ ] T025 [US2] Run `tests/e2e/portless_opaque_test.go` SC-006 — opaque (UUID) Agent with explicit-port registered `redirect_uri` matched against a different ephemeral port; MUST pass confirming FR-007 (port-ignore applies to all flows, not just CIMD)
...
- [ ] T026 [US3] Run US3 E2E scenarios 1–3 — non-loopback URI with port mismatch MUST be rejected; exact match MUST succeed; confirms FR-002 and SR-001
- [ ] T027 [US3] Run the full existing 028 E2E test suite (`ginkgo -v ./tests/e2e/cimd_authorization_test.go ./tests/e2e/cimd_ssrf_test.go ./tests/e2e/cimd_caching_test.go ./tests/e2e/cimd_consent_test.go ./tests/e2e/cimd_metadata_test.go ./tests/e2e/cimd_flow_test.go`) — zero regressions; all previously passing tests remain green (SC-004)
...
- [ ] T028 Run Playwright E2E suite including the SC-005 amendment in `tests/e2e/frontend/consent_flow_test.go` — loopback warning displayed for portless-registered loopback redirect URI; screenshot saved to `tests/e2e/screenshots/consent_loopback_warning_portless.png` with descriptive filename
- [ ] T029 [P] Update ARCHITECTURE.md redirect URI validation section (or OAuth2 authorization server section) to document: loopback hosts (`localhost`, `127.0.0.1`) use port-agnostic matching per RFC 8252 §7.3; non-loopback hosts require exact four-component match; IPv6 `::1` deferred
...
- [ ] TN01 Verify Phase 2a complete — no new glossary terms required; confirmed in T002
- [ ] TN02 Verify Phase 2b complete — no config changes; confirmed in T003
- [ ] TN03 Verify Phase 2c complete — no API changes; confirmed in T004
- [ ] TN04 Verify Phase 2d complete — no DB changes; confirmed in T005
- [ ] TN05 Verify Phase 2e complete — Playwright test scope confirmed, no new design system components needed
- [ ] TN06 Verify E2E acceptance tests written for ALL 12 spec scenarios in `tests/e2e/cimd_redirect_uri_test.go` and `tests/e2e/portless_opaque_test.go` (Principle XIII)
- [ ] TN07 Verify E2E tests contained detailed, realistic assertions and failed semantically before implementation — red phase output recorded in T011 (Principle XIII)
- [ ] TN08 Verify Playwright E2E test added in `tests/e2e/frontend/consent_flow_test.go` and screenshot configured for `tests/e2e/screenshots/consent_loopback_warning_portless.png` (Principle XIII)

#### Implementation Phase Verification [MANDATORY]

**API & Documentation** (Principles IV, X):
- [ ] TN09 Confirm no OpenAPI spec changes were made — API surface is unchanged

**Architecture & Documentation** (Principle II):
- [ ] TN10 Verify ARCHITECTURE.md updated with redirect URI loopback port exception (T029)
- [ ] TN11 Verify no ADR was required — decision is a RFC compliance correction fully documented in `specs/028b-portless-registration/spec.md` and `research.md`

**Configuration** (Principle VII):
- [ ] TN12 Confirm no configuration changes — Helm chart update not required

**Database & Persistence** (Principle IX):
- [ ] TN13 Confirm no migrations — no persistence changes

**Security** (Principles I, III):
- [ ] TN14 [P] Verify port-ignore exception is strictly scoped to `localhost` and `127.0.0.1` — non-loopback validation unchanged (SR-001)
- [ ] TN15 [P] Verify only `net/url` stdlib used in `MatchesRedirectURI` — no custom cryptography, no third-party deps (Principle III)

**Architecture Patterns** (Principle VI):
- [ ] TN16 Verify `MatchesRedirectURI` is in `internal/domain/storage/agent.go` (domain layer) — no adapter layer changes, hexagonal boundaries preserved

**Testing** (Principle VIII):
- [ ] TN17 Verify `TestMatchesRedirectURI` was written before `MatchesRedirectURI` was implemented — unit test red phase confirmed in T016
- [ ] TN18 Verify `TestMatchesRedirectURI` covers all 12 cases from `quickstart.md` and changed minimally after implementation

**E2E Acceptance Testing** (Principle XIII):
- [ ] TN19 Verify each `It()` block in `tests/e2e/cimd_redirect_uri_test.go` maps 1:1 to exactly one acceptance scenario from `spec.md` — 10 scenarios covered
- [ ] TN20 Verify `tests/e2e/portless_opaque_test.go` maps 1:1 to SC-006
- [ ] TN21 Verify all E2E test files include `// Scenario X from specs/028b-portless-registration/spec.md` comment references
- [ ] TN22 Run full E2E suite: `ginkgo -v ./tests/e2e/` — all tests pass
- [ ] TN23 Verify Playwright E2E tests in `tests/e2e/frontend/` pass and screenshot saved with correct filename

**Final verification**:
- [ ] TN24 Run `just check` — fmt, vet, lint, and all unit tests pass with no errors
