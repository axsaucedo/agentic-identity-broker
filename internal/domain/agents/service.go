...
// Create validates and persists a new agent. When clientID is empty, it is auto-generated
// from the agent's UUID (ADR 017). When multiAgentEnabled is false, client_id uniqueness
// is enforced.
func (s *Service) Create(ctx context.Context, agent *storage.Agent) error {
	...
	autoGenerated := agent.ClientID.IsZero()
	if autoGenerated {
		agent.ClientID = id.ClientID(agent.ID.String())
	}
	...
	// Enforce uniqueness when multi-agent mode is off.
	// Auto-generated UUIDs are inherently unique — skip the check.
	if !s.multiAgentEnabled && !autoGenerated {
