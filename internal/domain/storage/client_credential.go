package storage

import (
	"time"

	"github.com/agentic-identity-broker/agentic-identity-broker/internal/domain/id"
)

// ClientCredential represents OAuth2 client credentials generated by the broker
// and bound to exactly one Agent. Lifecycle: created on demand via Admin API,
// replaced atomically on rotation, cascade-deleted when the associated Agent is deleted.
type ClientCredential struct {
	ID         id.CredentialID `json:"id" db:"id"`
	AgentID    id.AgentID      `json:"agent_id" db:"agent_id"`
	SecretHash string          `json:"-" db:"secret_hash"` // Never serialize
	CreatedAt  time.Time       `json:"created_at" db:"created_at"`
	RotatedAt  *time.Time      `json:"rotated_at,omitempty" db:"rotated_at"`
}

// Validate validates the ClientCredential fields.
func (c *ClientCredential) Validate() error {
	if c.AgentID.IsZero() {
		return NewStorageError("ClientCredential.Validate", ErrorKindValidation, nil, "agent_id is required")
	}
	if c.SecretHash == "" {
		return NewStorageError("ClientCredential.Validate", ErrorKindValidation, nil, "secret_hash is required")
	}
	return nil
}
