...
// TestVerifyIssuer_Match tests successful issuer verification
func TestVerifyIssuer_Match(t *testing.T) {
	...
	err = validator.VerifyIssuer(token, "https://auth.example.com")
	assert.NoError(t, err)
}

// TestVerifyIssuer_Mismatch tests rejection of mismatched issuer
func TestVerifyIssuer_Mismatch(t *testing.T) {
	token, err := jwt.NewBuilder().
...
// TestVerifyAudience_Single tests successful audience verification with single audience
func TestVerifyAudience_Single(t *testing.T) {
	token, err := jwt.NewBuilder().
		Audience([]string{"broker-id"}).
		Build()
	require.NoError(t, err)
	...
	err = validator.VerifyAudience(token, "broker-id")
	assert.NoError(t, err)
}

// TestVerifyAudience_Multiple tests successful audience verification with multiple audiences
func TestVerifyAudience_Multiple(t *testing.T) {
	token, err := jwt.NewBuilder().
		Audience([]string{"service-1", "broker-id", "service-2"}).
		Build()
	require.NoError(t, err)
...
// TestVerifyAudience_NoMatch tests rejection when broker audience not in list
func TestVerifyAudience_NoMatch(t *testing.T) {
	token, err := jwt.NewBuilder().
		Audience([]string{"service-1", "service-2"}).
		Build()
	require.NoError(t, err)
...
// TestVerifyExpiration_Valid tests successful expiration verification
func TestVerifyExpiration_Valid(t *testing.T) {
	now := time.Now().UTC()
	token, err := jwt.NewBuilder().
	...
	err = validator.VerifyExpiration(token, 0)
	assert.NoError(t, err)
}

// TestVerifyExpiration_Expired tests rejection of expired token
func TestVerifyExpiration_Expired(t *testing.T) {
	now := time.Now().UTC()
	token, err := jwt.NewBuilder().
		Expiration(now.Add(-time.Hour)).
		Build()
...
// TestVerifyExpiration_WithinClockSkew tests acceptance of expired token within clock skew
func TestVerifyExpiration_WithinClockSkew(t *testing.T) {
	now := time.Now().UTC()
	token, err := jwt.NewBuilder().
	...
	err = validator.VerifyExpiration(token, 60*time.Second)
	assert.NoError(t, err)
}
