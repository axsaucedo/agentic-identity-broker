...
**OAuthServerMode**: Enumeration of valid OAuth server modes. Three values: `proxy` (forward all requests to upstream OAuth2 server), `local` (issue tokens locally, replaces the deprecated `issue_token` mode), and `hybrid` (accept all client modes — proxy, CIMD, and local — within a single broker). Stored as a string in `OAuth2AuthServerConfig.Mode`. The string `issue_token` is rejected with a deprecation error at config validation time (fail-closed). Located in `internal/ports/config.go`.

**ClientMode**: Property-based classification of a resolved Agent, derived exclusively from the agent's existing fields with no new database columns. Three values: `ProxyClient` (Agent.ClientID is set — requests are forwarded to an upstream OAuth2 server), `CIMDClient` (Agent.ClientURIs is set and Agent.ClientID is nil — tokens issued locally via CIMD), `LocalClient` (neither ClientID nor ClientURIs set — tokens issued locally). The two conditions are mutually exclusive by validation: an agent may not have both ClientID and ClientURIs set. Computed via `Agent.ClientMode()` method. Located in `internal/domain/storage/agent.go`.
...
**Mode Strategy Pattern**: The mechanism by which OAuth server mode drives the entire request handling topology at startup time rather than via runtime branching. The builder selects and wires the appropriate `ModeStrategy` (domain) and `AuthorizationProceedStrategy`/`TokenGrantStrategy` (adapters) based on the configured mode. For hybrid mode, both proxy and local strategies are created and wrapped in dispatching strategies that delegate per-request based on `agent.ClientMode()`. Handlers never inspect the configured mode string — they receive pre-wired strategies. This satisfies the constraint: no `if mode ==` checks in handler or service code.
...
**ClientMode**: Enum (`internal/domain/storage`) classifying an Agent at request time based on its registered identifiers. `ProxyClient` — has a `ClientID`; requests forwarded to upstream. `LocalClient` — no `ClientID` and no `ClientURIs`; tokens minted locally. `CIMDClient` — has `ClientURIs` but no `ClientID`; tokens minted locally after CIMD document fetch. `AmbiguousClient` — has both `ClientID` and `ClientURIs`; rejected with `unauthorized_client`. Computed by `Agent.ClientMode()` — never stored.
